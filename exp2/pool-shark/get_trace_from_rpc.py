...
if __name__ == '__main__':
    root_path = "/home/xcrab/MyWorkspace/ExperimentDapp/experiments/exp2_vulnerability/pool-shark"
    dirs = os.listdir(root_path)
    for version in dirs:
        if version == "v1" or version == "temp":
            continue
        ...
        for id,t_file in enumerate(t_files):
            if t_file.find("helper") is not -1:
                continue
            print(version)
            if t_file.find("util") is not -1:
                continue
            petflag = False
            print("gannache")
            ...
            cpcmd = "cp ./ganachi-out ./trainTrace/%d_ganachi" % id
            os.system(cpcmd)

            f = open("./trainTrace/%d_contracts" % id, "w+")
